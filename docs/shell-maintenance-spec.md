...
- Mục đích: chạy trực tiếp chuỗi lệnh deploy tự động mà không cần thông qua /sh
- Cách hoạt động:
  - Tự động lấy cấu hình sequence `deploy` từ `config/whitelist.js`.
  - Thực thi tuần tự các bước: `git pull origin main`, `npm install`, `npm run build`, và `pm2 restart all`.
  - Trả kết quả chi tiết dưới dạng code block HTML (nếu kết quả quá dài sẽ gửi dưới dạng file đính kèm).
- Trạng thái: `implemented`
