...
| Shell | `/sh [lệnh]` chạy lệnh shell theo whitelist | implemented | Chỉ cho phép các lệnh trong whitelist; từ chối tức thì nếu không khớp; có sẵn chuỗi deploy cố định |
...
| Maintenance | `/stop` process PM2 | implemented | Dùng `PM2_PROCESS_NAME` hoặc tên mặc định của bot |
...
- `/stop`:
	- Dừng process PM2 của bot theo tên process
...
| Maintenance | Stop process | implemented | Dùng tên process PM2 mặc định hoặc cấu hình riêng |
...
2. Lớp bảo trì an toàn: dọn cache, xoá file tạm, xoay log, dừng service đã whitelist.
3. Lớp thao tác nhạy cảm: chạy shell tùy ý, `nano`, sửa file hệ thống, reboot toàn máy.
...
6. Chỉ dùng stop trên process PM2 đã được định danh rõ ràng.
