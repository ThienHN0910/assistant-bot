...
function enrichInspection(res) {
  return {
    ...res,
    category: res.category || res.type,
    description: res.description || res.summary,
    suggestedTarget: res.suggestedTarget || res.defaultTarget,
    supportedTargets: res.supportedTargets || res.targets || [],
    frontendPath: res.frontend?.dir,
    frontendFramework: res.frontend?.frameworks?.join(', '),
    backendPath: res.backend?.dir,
    backendFramework: res.backend?.frameworks?.join(', '),
  };
}
