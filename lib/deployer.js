...
const repoInspector = require('./repoInspector');
const nodeManager = require('./nodeManager');
const nodeClient = require('./nodeClient');

async function downloadGithubRepoZip(owner, repo, destPath, client = axios) {
  const url = `https://api.github.com/repos/${owner}/${repo}/zipball`;
  const response = await client.get(url, {
    responseType: 'arraybuffer',
    headers: {
      Accept: 'application/vnd.github.v3+json',
      'User-Agent': 'assistant-bot-downloader',
    },
    timeout: 30000,
  });
...
function getValidTargetsForSource(source, config, repoType = null) {
  const active = getActiveTargets(config);
  if (source === 'github_public') {
    return active.filter((t) => t === 'vercel' || t === 'render' || (t === 'vps' && repoType === 'static_pure'));
  }
...
async function saveRegistry(registryPath, items) {
  ...
  try {
    ...
    try {
      ...
      if (process.platform === 'win32' || renameErr.code === 'EXDEV' || renameErr.code === 'EPERM') {
        await fs.copyFile(tempPath, registryPath);
...
async function deployUnlocked(
  { source, sourcePath, repoUrl, projectName, target, subdomain, port, nodeId },
  config,
  deps = {}
) {
  ...
  const depDownloader = deps.downloadGithubZip || downloadGithubRepoZip;
  ...
  if (source === 'github_public' && target === 'vps') {
    if (!repoUrl) {
      throw new Error('VPS chỉ hỗ trợ web tĩnh thuần (HTML/CSS/JS không build). Không hỗ trợ deploy link GitHub public trực tiếp lên VPS khi thiếu repoUrl.');
    }
    let inspection = null;
    try {
      inspection = await depInspector.inspectRepo(repoUrl, { client: deps.client });
    } catch (inspectErr) {
      throw new Error(`VPS chỉ hỗ trợ web tĩnh thuần (HTML/CSS/JS không build). Không hỗ trợ deploy link GitHub public trực tiếp lên VPS: ${inspectErr.message}`);
    }
    if (inspection?.type !== 'static_pure') {
      throw new Error('VPS chỉ hỗ trợ web tĩnh thuần (HTML/CSS/JS không build), không hỗ trợ build trực tiếp trên VPS. Không hỗ trợ deploy link GitHub public trực tiếp lên VPS đối với dự án yêu cầu build.');
    }
    const uploadDir = config.uploadDir || path.resolve(process.cwd(), 'data/uploads');
    const destZip = path.join(uploadDir, `${projectName || 'app'}-gh.zip`);
    await depDownloader(inspection.owner, inspection.repo, destZip, deps.client || axios);
    effectiveSourcePath = destZip;
  }
  ...
  if (target === 'vps') {
    selectedNode = await depNodeManager.getNode(nodeId || 'gcp-master', config);
    if (!selectedNode) {
      throw new Error(`Node '${nodeId || 'gcp-master'}' not found`);
    }
  }

  if (target === 'vps' && !existing && selectedNode?.isLocal && config.webDeployDir) {
  ...
  let vpsIp = null;
  if (target === 'vps') {
    vpsIp = selectedNode.ip;
    if (selectedNode.isLocal && (!vpsIp || vpsIp === '127.0.0.1')) {
      try {
        vpsIp = await getVpsPublicIp(config, deps.client || axios);
      } catch (err) {
        if (!vpsIp) throw err;
      }
    }
    if (!vpsIp) throw new Error('Unable to determine VPS public IP');
  }
  ...
  if ((!existing || ['failed_origin', 'provisioning_origin'].includes(existing.status)) && target === 'vps') {
    ...
    if (existing) {
      try {
        if (selectedNode.isLocal) {
          ...
          const cleanup = await depNodeClient.undeploy(selectedNode, sanitizedName, deps.client || axios);
          if (!cleanup?.ok) throw new Error('Previous incomplete VPS origin cleanup was not confirmed');
        }
      } catch (error) {
    ...
    if (selectedNode.isLocal) {
      try {
        vpsRes = await depSandbox.deployProject(effectiveSourcePath, sanitizedName, port, config, effectiveSubdomain);
      } catch (error) {
      ...
      try {
        const zipBuffer = Buffer.isBuffer(effectiveSourcePath)
          ? effectiveSourcePath
          : await fs.readFile(effectiveSourcePath);
        vpsRes = await depNodeClient.deploy(
          selectedNode,
          {
        ...
        if (source === 'github_public' && typeof effectiveSourcePath === 'string') {
          await fs.rm(effectiveSourcePath, { force: true }).catch(() => {});
        }
      } catch (error) {
        if (source === 'github_public' && typeof effectiveSourcePath === 'string') {
          await fs.rm(effectiveSourcePath, { force: true }).catch(() => {});
        ...
        let cleanupError = null;
        try {
          await depNodeClient.undeploy(selectedNode, sanitizedName, deps.client || axios).catch(() => {});
        } catch (failure) {
          cleanupError = failure.message;
        }
        const failed = {
      ...
      if (!vpsRes?.ok) throw new Error(vpsRes?.error || 'VPS remote origin deployment failed');
    }

    const depData = vpsRes.deployment || vpsRes;
    deployResult = {
      id: sanitizedName,
      name: sanitizedName,
      target: 'vps',
      ...
      source: source || 'zip_upload',
      sourceDetail: path.basename(sourcePath || sanitizedName),
      domain: depData.domain || fullDomain,
      url: depData.url || `https://${fullDomain}`,
      port: depData.type === 'backend' ? depData.port : null,
      type: depData.type || 'static',
    ...
    let vercelSourcePath = sourcePath;
    let tempDir = null;
    if (source === 'github_public' && repoUrl && !vercelSourcePath) {
      try {
        const { owner, repo } = repoInspector.parseGithubUrl(repoUrl);
        tempDir = path.join(os.tmpdir(), `vcl-gh-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`);
        await fs.mkdir(tempDir, { recursive: true });
        const destZip = path.join(tempDir, `${repo}.zip`);
        await depDownloader(owner, repo, destZip, deps.client || axios);
        vercelSourcePath = destZip;
      } catch (dlErr) {
        console.warn(`[VERCEL_GITHUB_DOWNLOAD_FALLBACK] Could not download zipball: ${dlErr.message}`);
      }
    }

    let vercelRes;
    try {
      vercelRes = await depVercel.deploy(
        {
          projectName: sanitizedName,
          subdomain: effectiveSubdomain,
          source,
          sourcePath: vercelSourcePath,
          repoUrl,
        },
        config,
        deps.client || axios
  ...
  if (selectedNode && !deployResult.nodeId) deployResult.nodeId = selectedNode.id;
...
async function undeployUnlocked(projectName, config, deps = {}) {
  ...
  if (depCloudflare.isEnabled(config) && !found.dnsRecordId) {
    const incomplete = ['failed_origin', 'failed_dns', 'cleanup_pending'].includes(found.status);
  ...
  try {
    let originRes;
    if (target === 'vps') {
      const nodeId = found.nodeId || 'gcp-master';
      const node = await depNodeManager.getNode(nodeId, config);
      if (!node && nodeId !== 'gcp-master') {
        throw new Error(`Node '${nodeId}' not found for deployment ${actualName}`);
      }
      if (node && !node.isLocal) {
        originRes = await depNodeClient.undeploy(node, actualName, deps.client || axios);
      } else {
        originRes = await depSandbox.removeProject(actualName, config);
      }
    }
